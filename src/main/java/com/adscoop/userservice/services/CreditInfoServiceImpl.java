...
public class CreditInfoServiceImpl implements CreditInfoService {
	...
	public Iterable<Map<String, Object>> getCreditInfoByUserId(Long s) throws IOException {
		try {
			return connectionSource.query("", Collections.emptyMap());
		} catch (Exception e) {
	...
	public Observable<Map<String, Object>> findByCypher(String cypherQuery) throws IOException {
		try {
			return Observable.from(connectionSource.query(cypherQuery, Collections.emptyMap()));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public CreditInfo findByUserId(Long id) throws Exception {
		...
		return connectionSource.queryForObject(CreditInfo.class,
				"match (c)-[:CREDIT_BELONGS_TO_USER]->(u) where u.id=" + id, Collections.emptyMap());
	}

	@Override
	public CreditInfo findByUserToken(String token) {
		try {
			return connectionSource.queryForObject(CreditInfo.class, "", Collections.emptyMap());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
