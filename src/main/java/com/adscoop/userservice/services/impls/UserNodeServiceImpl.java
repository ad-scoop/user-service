...
public class UserNodeServiceImpl implements IUser{
    ...
    public Optional<UserNode> findByUserNameAndPassword(String username, String password) throws IOException {
        try {
            return Optional.of(session().queryForObject(UserNode.class,"match (a:UserNode) where a.username='"+username+"' and  a.password='"+password+"' return a",Collections.EMPTY_MAP));
        } catch (Exception e) {
            e.printStackTrace();
        }
return null;
...
System.err.print(e);
    ...
    public UserNode findByToken(String token) throws IOException {
            try {
                return session().queryForObject(UserNode.class, "MATCH (n:UserNode) where n.token='"+token+"' RETURN n", Collections.EMPTY_MAP);
            } catch (Exception e) {
                e.printStackTrace();
            }
            return  null;
